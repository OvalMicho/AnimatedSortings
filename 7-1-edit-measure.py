...
try:
    ...
    settings= open("settings.txt", "at")
    settings.write("Средняя частота дискретизации: {} Гц ".format(len(t)/(t[len(t)-1])) + '\n')
    settings.write("Шаг квантования АЦП: {} ".format(3.3/256) + '\n')
    settings.close
    plt.plot(t,V)
    plt.xlabel("t,Сек")
    plt.ylabel("V,Вольт")
